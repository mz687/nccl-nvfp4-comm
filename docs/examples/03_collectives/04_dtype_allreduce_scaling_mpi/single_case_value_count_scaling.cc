/*
 * SPDX-License-Identifier: Apache-2.0
 */

#include <cuda_runtime.h>
#if CUDART_VERSION >= 11000
#include <cuda_bf16.h>
#endif
#include <cuda_fp16.h>
#include <mpi.h>
#include <nccl.h>

#include <cmath>
#include <cctype>
#include <cstdint>
#include <cstdio>
#include <cstdlib>
#include <sstream>
#include <string>
#include <vector>

#define MPI_CHECK(cmd)                                                         \
  do {                                                                         \
    int err = (cmd);                                                           \
    if (err != MPI_SUCCESS) {                                                  \
      char errStr[MPI_MAX_ERROR_STRING];                                       \
      int len = 0;                                                             \
      MPI_Error_string(err, errStr, &len);                                     \
      std::fprintf(stderr, "MPI failure %s:%d '%.*s'\n", __FILE__, __LINE__,  \
                   len, errStr);                                               \
      MPI_Abort(MPI_COMM_WORLD, EXIT_FAILURE);                                 \
    }                                                                          \
  } while (0)

#define NCCL_CHECK(cmd)                                                        \
  do {                                                                         \
    ncclResult_t res = (cmd);                                                  \
    if (res != ncclSuccess) {                                                  \
      std::fprintf(stderr, "NCCL failure %s:%d '%s'\n", __FILE__, __LINE__,    \
                   ncclGetErrorString(res));                                   \
      std::fprintf(stderr, "Failed NCCL operation: %s\n", #cmd);              \
      MPI_Abort(MPI_COMM_WORLD, EXIT_FAILURE);                                 \
    }                                                                          \
  } while (0)

#define CUDA_CHECK(cmd)                                                        \
  do {                                                                         \
    cudaError_t err = (cmd);                                                   \
    if (err != cudaSuccess) {                                                  \
      std::fprintf(stderr, "CUDA failure %s:%d '%s'\n", __FILE__, __LINE__,    \
                   cudaGetErrorString(err));                                   \
      std::fprintf(stderr, "Failed CUDA operation: %s\n", #cmd);              \
      MPI_Abort(MPI_COMM_WORLD, EXIT_FAILURE);                                 \
    }                                                                          \
  } while (0)

namespace {

constexpr size_t kNvfp4BlockElts = 16;
constexpr size_t kNvfp4BlockBytes = 10;
constexpr size_t kNvfp4Alignment = 16;

enum class CaseKind { kNative, kNvfp4, kNvfp4RawU8 };

struct DTypeCase {
  std::string label;
  CaseKind kind;
  ncclDataType_t ncclType;
  size_t typeBytes;
};

struct Options {
  std::string dtypeLabel = "fp32";
  size_t valueCount = 128ULL << 20;
  int warmupSteps = 5;
  int measuredSteps = 50;
  bool csv = true;
};

std::string toLower(std::string value) {
  for (char& c : value) c = static_cast<char>(std::tolower(static_cast<unsigned char>(c)));
  return value;
}

bool endsWith(const std::string& value, const std::string& suffix) {
  return value.size() >= suffix.size() &&
         value.compare(value.size() - suffix.size(), suffix.size(), suffix) == 0;
}

size_t parseScaledCount(const std::string& text) {
  std::string value = toLower(text);
  while (!value.empty() && std::isspace(static_cast<unsigned char>(value.back()))) value.pop_back();
  size_t start = 0;
  while (start < value.size() && std::isspace(static_cast<unsigned char>(value[start]))) ++start;
  value = value.substr(start);
  if (value.empty()) {
    std::fprintf(stderr, "empty value count\n");
    std::exit(EXIT_FAILURE);
  }

  unsigned long long multiplier = 1;
  if (endsWith(value, "g")) {
    multiplier = 1ULL << 30;
    value.resize(value.size() - 1);
  } else if (endsWith(value, "m")) {
    multiplier = 1ULL << 20;
    value.resize(value.size() - 1);
  } else if (endsWith(value, "k")) {
    multiplier = 1ULL << 10;
    value.resize(value.size() - 1);
  }

  char* end = nullptr;
  double number = std::strtod(value.c_str(), &end);
  if (end == value.c_str() || *end != '\0' || number <= 0.0) {
    std::fprintf(stderr, "invalid value count '%s'\n", text.c_str());
    std::exit(EXIT_FAILURE);
  }
  long double count = static_cast<long double>(number) * static_cast<long double>(multiplier);
  if (count <= 0.0L || count > static_cast<long double>(~0ULL)) {
    std::fprintf(stderr, "value count overflow for '%s'\n", text.c_str());
    std::exit(EXIT_FAILURE);
  }
  return static_cast<size_t>(count);
}

std::string formatCount(size_t count) {
  char buffer[64];
  if (count >= (1ULL << 30)) {
    double value = static_cast<double>(count) / static_cast<double>(1ULL << 30);
    std::snprintf(buffer, sizeof(buffer), (count % (1ULL << 30)) == 0 ? "%.0fG" : "%.2fG", value);
  } else if (count >= (1ULL << 20)) {
    double value = static_cast<double>(count) / static_cast<double>(1ULL << 20);
    std::snprintf(buffer, sizeof(buffer), (count % (1ULL << 20)) == 0 ? "%.0fM" : "%.2fM", value);
  } else if (count >= (1ULL << 10)) {
    double value = static_cast<double>(count) / static_cast<double>(1ULL << 10);
    std::snprintf(buffer, sizeof(buffer), (count % (1ULL << 10)) == 0 ? "%.0fK" : "%.2fK", value);
  } else {
    std::snprintf(buffer, sizeof(buffer), "%zu", count);
  }
  return std::string(buffer);
}

void printUsage(const char* prog) {
  std::fprintf(stderr,
      "Usage: %s --dtype fp32|bf16|fp16|fp8|nvfp4|nvfp4_u8 --value-count 128M|256M|512M|1G|2G "
      "[--warmup-steps 5] [--measured-steps 50] [--csv]\n",
      prog);
}

Options parseOptions(int argc, char** argv) {
  Options opts;
  for (int i = 1; i < argc; ++i) {
    std::string arg(argv[i]);
    auto requireValue = [&](const char* name)->const char* {
      if (i + 1 >= argc) {
        std::fprintf(stderr, "%s requires a value\n", name);
        printUsage(argv[0]);
        std::exit(EXIT_FAILURE);
      }
      return argv[++i];
    };
    if (arg == "--dtype") {
      opts.dtypeLabel = toLower(requireValue("--dtype"));
    } else if (arg == "--value-count" || arg == "--count") {
      opts.valueCount = parseScaledCount(requireValue(arg.c_str()));
    } else if (arg == "--warmup-steps" || arg == "--warmup") {
      opts.warmupSteps = std::atoi(requireValue(arg.c_str()));
    } else if (arg == "--measured-steps" || arg == "--steps") {
      opts.measuredSteps = std::atoi(requireValue(arg.c_str()));
    } else if (arg == "--csv") {
      opts.csv = true;
    } else if (arg == "--help" || arg == "-h") {
      printUsage(argv[0]);
      std::exit(EXIT_SUCCESS);
    } else {
      std::fprintf(stderr, "unknown argument '%s'\n", arg.c_str());
      printUsage(argv[0]);
      std::exit(EXIT_FAILURE);
    }
  }
  if (opts.warmupSteps < 0 || opts.measuredSteps <= 0) {
    std::fprintf(stderr, "warmup steps must be >= 0 and measured steps must be > 0\n");
    std::exit(EXIT_FAILURE);
  }
  return opts;
}

DTypeCase dtypeCaseFromLabel(const std::string& label) {
  std::string dtype = toLower(label);
  if (dtype == "fp32" || dtype == "float" || dtype == "float32") {
    return {"fp32", CaseKind::kNative, ncclFloat32, sizeof(float)};
  }
  if (dtype == "fp16" || dtype == "half" || dtype == "float16") {
    return {"fp16", CaseKind::kNative, ncclFloat16, sizeof(__half)};
  }
  if (dtype == "bf16" || dtype == "bfloat16") {
#if CUDART_VERSION >= 11000
    return {"bf16", CaseKind::kNative, ncclBfloat16, sizeof(__nv_bfloat16)};
#else
    std::fprintf(stderr, "bf16 requires CUDA 11 or newer\n");
    std::exit(EXIT_FAILURE);
#endif
  }
  if (dtype == "fp8" || dtype == "fp8e4m3" || dtype == "e4m3") {
#if CUDART_VERSION >= 11080
    return {"fp8", CaseKind::kNative, ncclFloat8e4m3, 1};
#else
    std::fprintf(stderr, "fp8 requires CUDA 11.8 or newer\n");
    std::exit(EXIT_FAILURE);
#endif
  }
  if (dtype == "nvfp4" || dtype == "fp4") {
    return {"nvfp4", CaseKind::kNvfp4, ncclFloat16, 0};
  }
  if (dtype == "nvfp4_u8" || dtype == "nvfp4-u8" || dtype == "packed_u8" || dtype == "packed-u8") {
    return {"nvfp4_u8", CaseKind::kNvfp4RawU8, ncclUint8, sizeof(uint8_t)};
  }
  std::fprintf(stderr, "unsupported dtype '%s'\n", label.c_str());
  std::exit(EXIT_FAILURE);
}

size_t nvfp4SectionBytes(size_t count) {
  size_t blocks = count / kNvfp4BlockElts + ((count % kNvfp4BlockElts) != 0 ? 1 : 0);
  size_t logicalBytes = blocks * kNvfp4BlockBytes;
  return (logicalBytes + kNvfp4Alignment - 1) & ~(kNvfp4Alignment - 1);
}

bool checkedMultiply(size_t a, size_t b, size_t* out) {
  if (a != 0 && b > static_cast<size_t>(-1) / a) return false;
  *out = a * b;
  return true;
}

bool estimateNvfp4AllReduceMemoryFromCount(size_t count, size_t* scratchBytesOut,
                                           size_t* totalBytesOut) {
  size_t sectionBytes = nvfp4SectionBytes(count);
  if (sectionBytes > static_cast<size_t>(-1) / 4) return false;
  *scratchBytesOut = sectionBytes * 2;
  *totalBytesOut = sectionBytes * 4;
  return true;
}

int getLocalRank(MPI_Comm comm) {
  int worldRank = 0;
  MPI_CHECK(MPI_Comm_rank(comm, &worldRank));
  MPI_Comm nodeComm;
  MPI_CHECK(MPI_Comm_split_type(comm, MPI_COMM_TYPE_SHARED, worldRank, MPI_INFO_NULL, &nodeComm));
  int localRank = 0;
  MPI_CHECK(MPI_Comm_rank(nodeComm, &localRank));
  MPI_CHECK(MPI_Comm_free(&nodeComm));
  return localRank;
}

double mean(const std::vector<double>& values) {
  double sum = 0.0;
  for (double value : values) sum += value;
  return values.empty() ? 0.0 : sum / static_cast<double>(values.size());
}

double sampleStddev(const std::vector<double>& values, double avg) {
  if (values.size() <= 1) return 0.0;
  double accum = 0.0;
  for (double value : values) {
    double delta = value - avg;
    accum += delta * delta;
  }
  return std::sqrt(accum / static_cast<double>(values.size() - 1));
}

struct CaseBuffers {
  void* send = nullptr;
  void* recv = nullptr;
  size_t bufferBytes = 0;
  size_t scratchBytes = 0;
};

CaseBuffers allocateCaseBuffers(const DTypeCase& dtypeCase, size_t valueCount, int worldRank, int worldSize,
                                cudaStream_t stream) {
  CaseBuffers buffers;
  size_t requiredBytes = 0;
  if (dtypeCase.kind == CaseKind::kNvfp4 || dtypeCase.kind == CaseKind::kNvfp4RawU8) {
    buffers.bufferBytes = nvfp4SectionBytes(valueCount);
    if (dtypeCase.kind == CaseKind::kNvfp4) {
      if (!estimateNvfp4AllReduceMemoryFromCount(valueCount, &buffers.scratchBytes, &requiredBytes)) {
        if (worldRank == 0) std::fprintf(stderr, "nvfp4 memory estimate overflow for value_count=%zu\n", valueCount);
        MPI_Abort(MPI_COMM_WORLD, EXIT_FAILURE);
      }
    } else {
      if (buffers.bufferBytes > static_cast<size_t>(-1) / 2) {
        if (worldRank == 0) std::fprintf(stderr, "nvfp4_u8 buffer size overflow for value_count=%zu\n", valueCount);
        MPI_Abort(MPI_COMM_WORLD, EXIT_FAILURE);
      }
      requiredBytes = buffers.bufferBytes * 2;
    }
  } else {
    if (!checkedMultiply(valueCount, dtypeCase.typeBytes, &buffers.bufferBytes) ||
        buffers.bufferBytes > static_cast<size_t>(-1) / 2) {
      if (worldRank == 0) std::fprintf(stderr, "buffer size overflow for dtype=%s value_count=%zu\n", dtypeCase.label.c_str(), valueCount);
      MPI_Abort(MPI_COMM_WORLD, EXIT_FAILURE);
    }
    requiredBytes = buffers.bufferBytes * 2;
  }

  size_t freeBytes = 0;
  size_t totalDeviceBytes = 0;
  CUDA_CHECK(cudaMemGetInfo(&freeBytes, &totalDeviceBytes));
  if (requiredBytes > freeBytes) {
    if (worldRank == 0) {
      std::fprintf(stderr, "insufficient GPU memory dtype=%s nranks=%d value_count=%zu required_bytes=%zu free_bytes=%zu\n",
                   dtypeCase.label.c_str(), worldSize, valueCount, requiredBytes, freeBytes);
    }
    MPI_Abort(MPI_COMM_WORLD, EXIT_FAILURE);
  }

  CUDA_CHECK(cudaMalloc(&buffers.send, buffers.bufferBytes));
  CUDA_CHECK(cudaMalloc(&buffers.recv, buffers.bufferBytes));
  CUDA_CHECK(cudaMemsetAsync(buffers.send, 0, buffers.bufferBytes, stream));
  CUDA_CHECK(cudaMemsetAsync(buffers.recv, 0, buffers.bufferBytes, stream));
  CUDA_CHECK(cudaStreamSynchronize(stream));
  return buffers;
}

void freeCaseBuffers(CaseBuffers* buffers) {
  CUDA_CHECK(cudaFree(buffers->recv));
  CUDA_CHECK(cudaFree(buffers->send));
  buffers->recv = nullptr;
  buffers->send = nullptr;
}

void launchAllReduce(const DTypeCase& dtypeCase, const CaseBuffers& buffers, size_t valueCount,
                     ncclComm_t comm, cudaStream_t stream) {
  if (dtypeCase.kind == CaseKind::kNvfp4) {
    NCCL_CHECK(ncclAllReduceNvfp4(buffers.send, buffers.recv, valueCount, dtypeCase.ncclType, ncclSum, comm, stream));
  } else if (dtypeCase.kind == CaseKind::kNvfp4RawU8) {
    NCCL_CHECK(ncclAllReduce(buffers.send, buffers.recv, buffers.bufferBytes, dtypeCase.ncclType, ncclSum, comm, stream));
  } else {
    NCCL_CHECK(ncclAllReduce(buffers.send, buffers.recv, valueCount, dtypeCase.ncclType, ncclSum, comm, stream));
  }
}

void printStepRow(const DTypeCase& dtypeCase, int worldSize, const Options& opts, const CaseBuffers& buffers,
                  int step, double globalMs) {
  double valueRate = static_cast<double>(opts.valueCount) / 1.0e9 / (globalMs / 1.0e3);
  double algBw = static_cast<double>(buffers.bufferBytes) / 1.0e9 / (globalMs / 1.0e3);
  double busBw = algBw * (2.0 * static_cast<double>(worldSize - 1) / static_cast<double>(worldSize));
  std::printf("value_step,%s,%d,%zu,%s,%zu,%zu,%d,%d,%.6f,%.6f,%.6f,%.6f,%zu\n",
              dtypeCase.label.c_str(), worldSize, opts.valueCount, formatCount(opts.valueCount).c_str(),
              buffers.bufferBytes, buffers.scratchBytes, opts.warmupSteps, step, globalMs,
              valueRate, algBw, busBw, dtypeCase.typeBytes);
}

void printSummaryRow(const DTypeCase& dtypeCase, int worldSize, const Options& opts, const CaseBuffers& buffers,
                     const std::vector<double>& measuredMs) {
  double avgMs = mean(measuredMs);
  double stdMs = sampleStddev(measuredMs, avgMs);
  double valueRate = static_cast<double>(opts.valueCount) / 1.0e9 / (avgMs / 1.0e3);
  double algBw = static_cast<double>(buffers.bufferBytes) / 1.0e9 / (avgMs / 1.0e3);
  double busBw = algBw * (2.0 * static_cast<double>(worldSize - 1) / static_cast<double>(worldSize));
  std::printf("value_summary,%s,%d,%zu,%s,%zu,%zu,%d,%d,%.6f,%.6f,%.6f,%.6f,%.6f,%zu\n",
              dtypeCase.label.c_str(), worldSize, opts.valueCount, formatCount(opts.valueCount).c_str(),
              buffers.bufferBytes, buffers.scratchBytes, opts.warmupSteps, opts.measuredSteps,
              avgMs, stdMs, valueRate, algBw, busBw, dtypeCase.typeBytes);
}

void runSingleCase(const DTypeCase& dtypeCase, const Options& opts, int worldRank, int worldSize,
                   ncclComm_t comm, cudaStream_t stream) {
  CaseBuffers buffers = allocateCaseBuffers(dtypeCase, opts.valueCount, worldRank, worldSize, stream);

  for (int step = 1; step <= opts.warmupSteps; ++step) {
    MPI_CHECK(MPI_Barrier(MPI_COMM_WORLD));
    launchAllReduce(dtypeCase, buffers, opts.valueCount, comm, stream);
    CUDA_CHECK(cudaStreamSynchronize(stream));
    if (worldRank == 0 && !opts.csv) {
      std::printf("warmup_step=%d/%d\n", step, opts.warmupSteps);
      std::fflush(stdout);
    }
  }

  std::vector<double> measuredMs;
  measuredMs.reserve(static_cast<size_t>(opts.measuredSteps));
  cudaEvent_t start = nullptr;
  cudaEvent_t stop = nullptr;
  CUDA_CHECK(cudaEventCreate(&start));
  CUDA_CHECK(cudaEventCreate(&stop));

  for (int step = 1; step <= opts.measuredSteps; ++step) {
    MPI_CHECK(MPI_Barrier(MPI_COMM_WORLD));
    CUDA_CHECK(cudaEventRecord(start, stream));
    launchAllReduce(dtypeCase, buffers, opts.valueCount, comm, stream);
    CUDA_CHECK(cudaEventRecord(stop, stream));
    CUDA_CHECK(cudaEventSynchronize(stop));

    float localMs = 0.0f;
    CUDA_CHECK(cudaEventElapsedTime(&localMs, start, stop));
    double localMsDouble = static_cast<double>(localMs);
    double globalMs = 0.0;
    MPI_CHECK(MPI_Allreduce(&localMsDouble, &globalMs, 1, MPI_DOUBLE, MPI_MAX, MPI_COMM_WORLD));
    measuredMs.push_back(globalMs);
    if (worldRank == 0) printStepRow(dtypeCase, worldSize, opts, buffers, step, globalMs);
  }

  if (worldRank == 0) printSummaryRow(dtypeCase, worldSize, opts, buffers, measuredMs);
  CUDA_CHECK(cudaEventDestroy(stop));
  CUDA_CHECK(cudaEventDestroy(start));
  freeCaseBuffers(&buffers);
}

}  // namespace

int main(int argc, char** argv) {
  MPI_CHECK(MPI_Init(&argc, &argv));

  int worldRank = 0;
  int worldSize = 0;
  MPI_CHECK(MPI_Comm_rank(MPI_COMM_WORLD, &worldRank));
  MPI_CHECK(MPI_Comm_size(MPI_COMM_WORLD, &worldSize));

  Options opts = parseOptions(argc, argv);
  DTypeCase dtypeCase = dtypeCaseFromLabel(opts.dtypeLabel);
  int localRank = getLocalRank(MPI_COMM_WORLD);

  int deviceCount = 0;
  CUDA_CHECK(cudaGetDeviceCount(&deviceCount));
  if (deviceCount == 0) {
    if (worldRank == 0) std::fprintf(stderr, "No CUDA devices found\n");
    MPI_Abort(MPI_COMM_WORLD, EXIT_FAILURE);
  }
  if (localRank >= deviceCount) {
    std::fprintf(stderr, "Rank %d requested local GPU %d but only %d device(s) are visible\n",
                 worldRank, localRank, deviceCount);
    MPI_Abort(MPI_COMM_WORLD, EXIT_FAILURE);
  }
  CUDA_CHECK(cudaSetDevice(localRank));

  cudaStream_t stream = nullptr;
  CUDA_CHECK(cudaStreamCreate(&stream));

  ncclUniqueId uniqueId;
  if (worldRank == 0) NCCL_CHECK(ncclGetUniqueId(&uniqueId));
  MPI_CHECK(MPI_Bcast(&uniqueId, NCCL_UNIQUE_ID_BYTES, MPI_CHAR, 0, MPI_COMM_WORLD));

  ncclComm_t comm = nullptr;
  NCCL_CHECK(ncclCommInitRank(&comm, worldSize, uniqueId, worldRank));

  if (worldRank == 0) {
    std::printf("single-case value-count allreduce starting: dtype=%s nranks=%d value_count=%zu label=%s warmup_steps=%d measured_steps=%d\n",
                dtypeCase.label.c_str(), worldSize, opts.valueCount, formatCount(opts.valueCount).c_str(),
                opts.warmupSteps, opts.measuredSteps);
    std::printf("kind,dtype,nranks,value_count,value_count_label,buffer_bytes,scratch_bytes,warmup_steps,step_or_steps,avg_ms_or_mean_ms,value_rate_gvals_or_std_ms,algbw_gbps_or_value_rate_gvals,busbw_gbps_or_algbw_gbps,type_bytes_or_busbw_gbps\n");
    std::fflush(stdout);
  }

  runSingleCase(dtypeCase, opts, worldRank, worldSize, comm, stream);

  CUDA_CHECK(cudaStreamSynchronize(stream));
  NCCL_CHECK(ncclCommFinalize(comm));
  NCCL_CHECK(ncclCommDestroy(comm));
  CUDA_CHECK(cudaStreamDestroy(stream));

  if (worldRank == 0) {
    std::printf("single-case value-count allreduce completed\n");
    std::fflush(stdout);
  }

  MPI_CHECK(MPI_Finalize());
  return EXIT_SUCCESS;
}
