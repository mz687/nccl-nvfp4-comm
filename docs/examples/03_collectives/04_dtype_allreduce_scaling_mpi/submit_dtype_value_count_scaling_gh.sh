...
export DTYPES="${DTYPES:-fp32,bf16,fp16,fp8,nvfp4}"
export VALUE_COUNTS="${VALUE_COUNTS:-128M,256M,512M,1G,2G}"
WARMUP="${WARMUP:-3}"
ITERS="${ITERS:-10}"
REPEATS="${REPEATS:-1}"
RUN_TAG="${RUN_TAG:-$(date +dtype_value_count_%Y%m%d_%H%M%S)}"
...
for nranks in ${RANKS_LIST}; do
  ...
  JOB_IDS+=("${job_id}")
  echo "submitted value-count scaling job: nranks=${nranks} job_id=${job_id}"
done
...
plot_output="$(sbatch \
  ...
  "${SCRIPT_DIR}/dtype_value_count_scaling_plot_gh.slurm" 2>&1)"
...
echo "submitted value-count plot job: job_id=${plot_job_id} run_tag=${RUN_TAG} dependency=${dependency}"
